...
function inkthemes_optionsframework_add_admin() {
    ...
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            ...
            inkthemes_reset_options($options,
                    'optionsframework');
    ...
    $of_page = add_theme_page($themename,
            'Theme Options',
            'edit_theme_options',
            'optionsframework',
    ...
    add_action("admin_print_scripts-$of_page",
            'inkthemes_load_only');
}

add_action('admin_menu',
        'inkthemes_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
...
function inkthemes_optionsframework_options_page() {
    ...
    <div class="trail-notify">
        <h1>Get Photomaker PRO!</h1>
        <p style="font-size:15px; line-height: 20px;">You are using the Photomaker free version theme. Upgrade to pro for extra features like home page slider, Portfolio, fullWidth, contact page templates and many other, multiple color options, premium theme support and much more.</p>
        <a href="http://www.inkthemes.com/wp-themes/photomaker-business-wordpress-theme/" target="blank">Upgrade to Photomaker PRO here >></a>
    </div>
...
function inkthemes_load_only() {
    ...
    wp_register_script('jquery-input-mask',
            get_template_directory_uri() . '/functions/js/jquery.maskedinput-1.2.2.js',
            array(
        'jquery'));
...
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action',
...
function inkthemes_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        $filename['name'] = preg_replace('/[^a-zA-Z0-9._\-]/',
                '',
                $filename['name']);

        $override['test_form'] = false;
        $override['action'] = 'wp_handle_upload';
        $uploaded_file = wp_handle_upload($filename,
                $override);

        $upload_tracking[] = $clickedID;
        inkthemes_update_option($clickedID,
                $uploaded_file['url']);
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            inkthemes_update_option($id,
                                    stripslashes($new_value));
                        }
                    }
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            inkthemes_update_option($multicheck_id,
                                    'false');
                        } else {
                            inkthemes_update_option($multicheck_id,
                                    'true');
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case "radio":
                ...
                break;
            case "checkbox":
                $output .= '<input id="' . esc_attr($value['id']) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" ' . checked($val,
                                1,
                                false) . ' />';
                $output .= '<label class="explain" for="' . esc_attr($value['id']) . '">' . wp_kses($explain_value,
                                $allowedtags) . '</label>';
                break;
            case "multicheck":
                foreach ($value['options'] as $key => $option) {
                    $checked = '';
                    $label = $option;
                    $option = preg_replace('/[^a-zA-Z0-9._\-]/',
                            '',
                            strtolower($key));
                    $id = $option_name . '-' . $value['id'] . '-' . $option;
                    $name = $option_name . '[' . $value['id'] . '][' . $option . ']';
                    if (isset($val[$option])) {
                        $checked = checked($val[$option],
                                1,
                                false);
                    }
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= inkthemes_optionsframework_uploader_function($value['id'],
                            $value['std'],
                            null);
                }
                break;
            ...
            case "heading":
                ...
                $jquery_click_hook = preg_replace("/[^A-Za-z0-9]/",
                        "",
...
function inkthemes_optionsframework_uploader_function($id, $std, $mod) {
    ...
    if ($mod != 'min') {
        ...
        $uploader .= '<input class=\'of-input\' name=\'' . $id . '\' id=\'' . $id . '_upload\' type=\'text\' value=\'' . str_replace("'",
                        "",
                        $val) . '\' />';
    }
